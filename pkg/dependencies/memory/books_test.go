...
func TestGetBooks(t *testing.T) {
	t.Skip()
	books, _ := NewBookRetriever().GetBooks()
	if len(books) == 0 {
...
func TestLoadJson(t *testing.T) {
	t.Skip()
	str := `<main className="flex flex-col items-center justify-between p-24">
	...
	book := parseBooks(node)
	bookJSON, err := json.MarshalIndent(book, "", "  ")
	if err != nil {
		log.Fatal(err)
	}

	t.Fatal(string(bookJSON))

}

func parseBooks(n *html.Node) []pkg.Book {
	var books []pkg.Book
	var currentBook pkg.Book
	var yearRead int
	...
	f = func(n *html.Node) {
		if n.Type == html.ElementNode && n.Data == "div" {
			if currentBook.Title != "" {
				currentBook.YearRead = yearRead
				books = append(books, currentBook)
				currentBook = pkg.Book{}
			}
		}
		if n.Type == html.ElementNode && n.Data == "a" {
			for _, attr := range n.Attr {
				if attr.Key == "href" {
					currentBook.Link = attr.Val
				}
			}
		}
		if n.Type == html.ElementNode && n.Data == "b" {
			currentBook.Title = n.FirstChild.Data
		}
		if n.Type == html.ElementNode && n.Data == "p" {
			switch {
			case currentBook.Author == "":
				currentBook.Author = n.FirstChild.Data
			case currentBook.Rating == 0:
				ratingStr := strings.TrimPrefix(n.FirstChild.Data, "Rating: ")
				ratingParts := strings.Split(ratingStr, "/")
				if len(ratingParts) > 0 {
					rating, err := strconv.Atoi(ratingParts[0])
					if err == nil {
						currentBook.Rating = rating
					}
				}
			case currentBook.Summary == "":
				currentBook.Summary = strings.ReplaceAll(strings.ReplaceAll(n.FirstChild.Data, "\n", ""), "\t", "")
			case currentBook.PersonalTake == "":
				currentBook.PersonalTake = strings.ReplaceAll(strings.ReplaceAll(n.FirstChild.Data, "\n", ""), "\t", "")
			}
		}
		if n.Type == html.ElementNode && n.Data == "h2" {
			if n.FirstChild != nil {
				yearRead, _ = strconv.Atoi(n.FirstChild.Data)
			}
		}
